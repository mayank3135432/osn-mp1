/*generated by GPT https://chatgpt.com/c/41137d7e-a044-4581-960e-46ca8bb4409a
...
int log_func(char** tokens, char* homedir, char** ptrprevdir, char* input){
    ...
    if(tokens[1] == NULL){
        ...
        execute_history_command(index, homedir, ptrprevdir, input, history_file);
    }
    return 0;
...
void execute_history_command(int index, char* homedir, char** ptrprevdir, char* input, char* history_file){
    ...
    while (fgets(buffer, MAX, fptr)) {
        if (++count == index) {
            ...
            execute_command(args, homedir, ptrprevdir, input);
            free(args);
            break;
        }
    }
